...
struct SQLTitleUpdate {
	SQLTitleUpdate(ULONGLONG titleUpdateId, wstring updateName, string path, int pathType, string filename, bool isBackup, ULONGLONG isBackupFor, bool isDisabled, string Date, string Hash, int Version) :
		titleUpdateId(titleUpdateId), updateName(updateName), path(path), pathType(pathType), filename(filename), isBackup(isBackup), isBackupFor(isBackupFor), isDisabled(isDisabled), Date(Date), Hash(Hash), Version(Version) {};
