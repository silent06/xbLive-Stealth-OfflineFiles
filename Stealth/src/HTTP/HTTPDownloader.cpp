...
// network changes.
void HTTPDownloader::handleNetworkConnected() {
	Resume();
}


void HTTPDownloader::handleNetworkDisconnected() {
	Pause();
}
...
HttpItem* HTTPDownloader::CreateHTTPItem(const CHAR* Url, const CHAR* OutputFilePath, REQUEST_METHOD RequestMethod)
{
	...
	//LOG("HTTPDownload", "Queue Size:  %d", m_LiveHttpItems.size());
	LOG_PRINT(StrEnc("HTTPDownload", "Queue Size:  %d", m_LiveHttpItems.size()));
	return item;
}
...
void HTTPDownloader::DestroyHTTPItem( HttpItem * item )
{
	...
	std::map<HttpItem* const,HttpItem* const>::iterator iter = m_LiveHttpItems.find(item);
	
	if( item ) {
		delete item;
	}
	m_LiveHttpItems.erase(iter);
}
...
void HTTPDownloader::AddToQueue(HttpItem* ref) {
	...
	if(NetworkMonitor::getInstance().HasInternetConnection()== FALSE)
		return;
...
void HTTPDownloader::DebugListDownloadQueue() {
	...
	for(DWORD x = HTTP_PRIORITY_START; x <= HTTP_PRIOIRTY_END; x++)
	{
		...
		for(DWORD y = 0; x < queue->size(); x++)
		{
			...
			//LOG("HTTPDownloader", "%s", currentItem->getSaveAsPath());
			LOG_PRINT(StrEnc("HTTPDownloader", "%s", currentItem->getSaveAsPath()));
		}
	}
}
...
void HTTPDownloader::Pause() {
	paused = TRUE;
	LOG_PRINT(StrEnc("HTTPDownloader", "HTTP Downloader Paused"));
	//LOG("HTTPDownloader","HTTP Downloader Paused");
}
void HTTPDownloader::Resume() {
	paused = FALSE;
	LOG_PRINT(StrEnc("HTTPDownloader", "HTTP Downloader Resumed"));
	//LOG("HTTPDownloader","HTTP Downloader Resumed");
}
BOOL HTTPDownloader::GetStatus() {
	return paused;
}
