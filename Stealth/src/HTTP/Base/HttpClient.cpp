...
HRESULT HttpClient::GET(HttpItem* Item) {
	...
	HttpHeader header(Item->getPath().c_str(), "GET", "HTTP/1.0");
	header.AddHeaderField("Host", m_serverName);
	header.AddHeaderField("User-Agent", "Silent Is your Hero!! @17559");
...
Restart:
	...
	if(result != ERROR_SUCCESS) {
		if(m_dwResponseCode == 302) {
			...
			//LOG("HttpClient", "302 Temporary Move, redirecting\r\n");
			LOG_PRINT(StrEnc("HttpClient", "302 Temporary Move, redirecting\r\n"));
			goto Restart;
		} else if (m_dwResponseCode == 404) {
			return result;
		} else if (m_dwResponseCode == 0) {
			LOG_PRINT(StrEnc("HttpClient", "HttpClient:ExecuteItem got a response code of 0"));
			//LOG("HTTPClient", "HttpClient:ExecuteItem got a response code of 0");
		} else {
			DebugBreak();
		}
	}
