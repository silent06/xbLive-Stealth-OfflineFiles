...
//	CloseHandle(hEvent);
	//LOG("Thread Finnish : %s, 0x%X",name.c_str(),(int)hThreadId);
}
...
void CThread::Terminate()
{
	if (this->hThread) {
		...
		{
			//LOG("CThread","CThread: Forcing termination of thread %s 0x%X",name.c_str(),(int)hThreadId);
...
void CThread::SetThreadName( char* threadName)
{
   ...
   //LOG("CThread", "This thread is named [%s]", threadName);
